...
def rec_709_trc_to_linear(value: float) -> float:
    ...
    Args:
        value (float): The input value in the range [0.0, 1.0].

    Returns:
        float: The linear light value in the range [0.0, 1.0].
    """
    if value < 0.081:
        return value / 4.5
    else:
...
def linear_to_rec_709_trc(value: float) -> float:
    ...
    Args:
        value (float): The input linear light value in the range [0.0, 1.0].

    Returns:
        float: The Rec. 709 transfer characteristic value in the range [0.0, 1.0].
    """
    if value < 0.018:
        return value * 4.5
    else:
...
def luma_range(bit_depth: int, is_full_range: bool) -> tuple[int, int]:
    ...
    Args:
        bit_depth (int): The bit depth (e.g., 8 or 10).
        is_full_range (bool): Whether the color range is full (True) or limited (False).

    Returns:
        tuple[int, int]: The minimum and maximum luma values.
    """
    if is_full_range:
        min_luma = 0
